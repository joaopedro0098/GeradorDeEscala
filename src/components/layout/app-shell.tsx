...
function buildAdminNav(context: AppShellContext): NavItem[] {
  if (!context.session) {
    return [
      { href: '/admin', label: 'Início', icon: <Home className="h-4 w-4" /> },
...
function buildMemberNav(): NavItem[] {
  return [
    { href: '/membro', label: 'Início', icon: <Home className="h-4 w-4" /> },
    {
    ...
    <div className="app-shell relative min-h-screen">
      ...
      <div className="flex min-h-screen">
        <aside className="glass-panel fixed inset-y-0 left-0 z-40 flex w-60 flex-col">
          <div className="flex items-center gap-3 px-3 pb-3 pt-[max(1rem,env(safe-area-inset-top))]">
            <div
              className="h-10 w-10 shrink-0 rounded-full border border-slate-200/80 bg-slate-100/90"
              aria-hidden="true"
            />
            <p className="font-display text-sm font-bold tracking-tight text-[var(--text-primary)]">
              Equipgestor
            </p>
          </div>
        ...
        <div className="ml-60 flex min-w-0 flex-1 flex-col">
          <header className="glass-card mx-4 mt-4 rounded-2xl px-5 py-4 sm:mx-6">
            <h1 className="font-display text-lg font-semibold text-[var(--text-primary)] sm:text-xl">
              {title}
            </h1>
          </header>
